...
void starttheChat()
{
        ...
        while(1){
                ...
                if(ret == 0){
						...
						//Keyboard inputs, fd points to standart input output
                        if(pollset[0].revents & POLLIN)
                        {

                                memset(outputBuff, 0,N);
                                bytesRead = read(pollset[0].fd, &outputBuff, (ssize_t)(sizeof(outputBuff)-1));
                                //3 consecutive new lines checked to send FIN
								if(bytesRead == 1){
                                        memset(outputBuff, 0,N);
                                        nullCtr++;
                                        if(nullCtr == 3){
						 ...
						 //Incoming information from socket connected to UDP port
...
int main(int argc, char *argv[]){
		...
		//a struct to keep up with the states
		...
		//start to poll the file descriptor
        starttheChat();
		//end of lifecycle
        close(listenfd);
        return 0;
